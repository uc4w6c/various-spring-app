...
public class AuthorizationServerConfig {
  @Bean
  @Order(1)
  public SecurityFilterChain authorizationServerSecurityFilterChain(HttpSecurity http, PasswordEncoder passwordEncoder)
      throws Exception {
  ...
  public PasswordEncoder passwordEncoder() {
    // これを設定すると{noop}がそのままsecretの値の一部として扱われてしまうため注意
  ...
  public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer() {
    return (context) -> {
        context.getClaims().claims(claims ->
            claims.put("token", UUID.randomUUID().toString()));
    };
  }
}
